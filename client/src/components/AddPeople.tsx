        ...
        <div className="fixed inset-0 p-5 flex justify-center items-center bg-black bg-opacity-40 z-50">
            ...
            <div className="bg-white w-96 max-h-[70vh] rounded-lg shadow-lg p-4 relative">
                ...
                <input
                    ...
                    onChange={(e) => { userSearch((e.target as HTMLInputElement).value) }}
                ...
                <div className="overflow-y-auto max-h-[50vh] space-y-1">
                    {
                        searchUserData <= 0 && search.length > 0 ?
                            <span className='text-xs flex justify-center items-center'>
                                User not found!!
                            </span> :
                            searchUserData.map((itm:user_info, idx) => (
                                <div key={idx} onClick={() => onSelectUser(itm)} className='rounded-lg bg-slate-300 p-2 flex items-center space-x-3 duration-500 border hover:cursor-pointer select-none hover:bg-slate-100'>
                                    <img src='' alt='img' className='h-10 aspect-[1] rounded-full bg-slate-600 ' />
                                    <h1 className='text-nowrap'>
                                        <p className='font-bold text-md'>{itm.name}</p>
                                        <p className='truncate w-40 text-xs ps-1'>{itm.email}</p>
                                    </h1>
                                </div>))
                    }
                </div>
